...
data/raw/base de datos VG/
    {ciudad}/               ← cdmx, guadalajara, monterrey
        {contaminante}/     ← CO, NO2, O3, PM2.5, SO2
            Datos SINAICA-{contaminante}-CH-{año}.csv
...
def _leer_csv_sinaica(path: Path) -> pd.DataFrame:
    """
    Lee un CSV individual de SINAICA y devuelve el promedio horario
...
def limpiar_datos(df: pd.DataFrame) -> pd.DataFrame:
    """
    PASO 2 — Limpieza y mejoras del DataFrame crudo.
    ...
    2. Reemplazar valores negativos por NaN
       → Concentraciones negativas son físicamente imposibles
...
def guardar_resultados(datos_limpios, datos_diarios, daily_wide, weekly_wide, monthly_wide):
    ...
    CSV para lectura humana, Parquet para carga rápida en la app.
    """
